...
See [**Increasing a Tenant's Resources**](add-tenant-resources.md) or [**Reducing a Tenant's Resources**](reduce-tenant-resources.md) for instructions regarding modifying resources.
...
* **UI Address** - Link for the tenant to access their own VergeOS UI. This field is **autopopulated** with the [**first External IP**](assign-ip-to-tenant.md) assigned to the tenant. If/when additional external IPs are assigned to a tenant, the **UI address can be changed** to one of the other IP addresses. Rules will **automatically** be updated by the system; *Apply Rules* to **both** the associated **External** network and **tenant** network if *UI Address* is modified.

* **UI FQDN** - Only applies when a [**Proxy**](https://app.gitbook.com/s/QZBMFpokMv2vWTIRbFzA/tenants/configuring-proxy) is being used. Select desired FQDN from the dropdown list.  
