...
{% hint style="info" %}
**What “TB” means in the VergeOS UI**
...
- Storage tier sizing reported in the GUI (see [Storage Tiers](storage-tiers.md))
- Virtual machine drive sizes and snapshots
- NAS volume sizes
- Tenant resource allocations and [usage reports](../tenants/tenant-usagereports.md)
- Backup and replication size reporting

Beyond the labeling convention, vSAN usable capacity is also affected by redundancy levels, which reduce usable capacity in exchange for fault tolerance. Deduplication, by contrast, generally increases usable capacity by collapsing duplicate blocks. These effects are independent of the unit-labeling behavior described on this page. See [vSAN Architecture](vsan-architecture.md) and [vSAN Redundancy Levels](vsan-redundancy-levels.md) for details.
...
- [vSAN Architecture](vsan-architecture.md) — how vSAN distributes and protects data
- [vSAN Redundancy Levels](vsan-redundancy-levels.md) — how redundancy affects usable capacity
- [Storage Tiers](storage-tiers.md) — performance and capacity tiering
- [Tenant Usage Reports](../tenants/tenant-usagereports.md) — interpreting per-tenant capacity reporting
