...
{% hint style="success" %}
**Most cluster setting changes will require a reboot of each cluster node.  A message will appear at the top of the cluster dashboard when a reboot is necessary.  Select the *Need Restart* option (within the reboot/restart message at the top of the cluster dashboard or left menu option) to manage reboots of the cluster nodes.  Always follow proper [Maintenance Mode procedures](../operations/maintenance-mode.md) when rebooting nodes to avoid workload disruptions.**
...
- From the top menu, navigate to  **Infrastructure** > **Clusters** > **double-click the desired cluster**
...
{% hint style="success" %}
**To accommodate migration/failover of virtual machines to another cluster using a lower class of CPU chip, *Default CPU type* can be changed to the lower class type to allow virtual machines to be ported to the older chip hardware without issue.**


{% endhint %}

{% hint style="warning" %}
**All nodes within a cluster should contain the same CPU hardware; mixed CPU types within the same cluster can cause performance and workload migration issues.**
...
{% hint style="warning" %}
**IMPORTANT: Although disabling CPU security mitigations can improve performance, this can be risky. Only select this option when you completely trust all guests running in this cluster and can be sure the workloads have no external vulnerabilities (e.g. airgapped systems).**
...
{% hint style="success" %}
**Unfiltered logs remain accessible via *Node Diagnostics* (Navigate to *Nodes* > double-click the desired node > select *Diagnostics* from the left menu.)**
...
{% hint style="info" %}
**Nested virtualization can involve security implications and issues with fair queuing/metering.**
...
{% hint style="info" %}
**If not enough vGPU devices are available, workloads may be temporarily powered off during maintenance.**
...
{% hint style="warning" %}
**Swap settings are specified during cluster installation. Swap setting changes will only apply to newly formatted disks.**
...
1. **Maximum Core Temperature (Celsius)** Establishes a peak temperature to use for VergeOS temperature monitoring (used in combination with the next setting: *Maximum Core Temperature Warning Threshold %*).
    ...
    {% hint style="info" %}
    **Non-server hardware and some legacy servers may not support this query; in such cases, the *custom* setting can be selected to establish a max temperature in which to base VergeOS warnings.**
...
3. **Critical Core Temperature (Celsius):** Defines temperature at which to trigger an error state in VergeOS. When reported hardware temperatures reach this temperature, the node status will turn to red and an error entry will appear in the system log.
