...
description: >-
  How to configure SR-IOV NIC passthrough in VergeOS, including creating
  resource groups, configuring virtual functions, VM guest setup, and sharing
  SR-IOV devices to tenants.
tags:
  - sriov
  - nic
  - passthrough
...
{% hint style="info" %}
**VergeOS provides built-in, virtualized network adapter capability for virtual machines. SR-IOV virtual function NICs are supported (version 4.13 and greater) for scenarios where direct NIC passthrough is a requirement. Where direct NIC passthrough is used, VergeOS networking is bypassed, losing inherent portability, redundancy, and other network features.**
{% endhint %}

## Host Installation/Configuration

{% hint style="danger" %}
**It is important to read and be familiar with** [**PCI Passthrough Risks and Precautions**](device-pass-overview.md#pci-passthrough-risksprecautions) **before making passthrough configurations.**
{% endhint %}

The following instructions will configure virtual function passthrough by automatically creating necessary resource rules for each set of virtual functions and attaching the rules to the selected resource group. For more information about how resource groups and resource rules work, see: [**Device Passthrough - Resource Rules**](device-pass-overview.md#resource-rules).
...
1. **Name**: label used to identify the resource group (i.e. device pool); use a descriptive name so that users are easily able to identify the type of device(s) in this group, e.g. _SR-IOV NIC vlan10_
2. **Type**: _**SR-IOV NIC**_.
3. **Description**: optional field to provide more administrative text about the resource group.
4. **Class**: _**Network**_. This field is only used to apply an associated icon to the resource group, and does not affect functionality.
...
* Check the _**User Defined**_ option for fields where a VM user should be allowed to override the specified setting; fields that do not have the User Defined option checked cannot be changed by the user when attaching the device to a VM.
* **Native VLAN**: (optional) VLAN tag for the virtual device.
* **VLAN Protocol**: _802.1Q_ (default) is the most widely used. _802.1ad_ provides support for QinQ (nesting of VLAN tags).
...
* **Virtual Link State**:\
  &#xNAN;_**Auto**_ - the VF state will simply mirror that of the associated physical device.\
  &#xNAN;_**Enable**_ - will allow the virtual device to communicate with other virtual devices on the same host even when the physical device is down.\
  &#xNAN;_**Disable**_ - packets sent by the virtual device are dropped by the hardware.
...
{% hint style="warning" %}
**Follow proper** [**Maintenance Mode**](../operations/maintenance-mode.md) **procedures when rebooting a node to avoid workload disruptions.**
{% endhint %}

The resource group dashboard contains the resource rules that were auto-generated for your selected devices. You can click an individual rule to view configuration detail. A system-created rule can be modified as needed; for example, the _Node_ filter can be changed to _-- None --_ to include matching devices from all nodes. Information regarding resource rules is available at: [**Device Passthrough Overview - Resource Rules**](device-pass-overview.md#resource-rules)
...
{% hint style="success" %}
**You can also attach devices to a VM via Resource Manager: from the Resource Group dashboard > double-click the desired SR-IOV Group > View Machine Devices > New; select the VM from the Machine dropdown list.**
{% endhint %}
...
* **SR-IOV NIC Settings**: options available here will depend on the _User Defined_ toggle selections defined in the SR-IOV Settings of the resource group (See **SR-IOV Settings** above.)
...
12. The device(s) will now be available as a resource group to attach to tenant VMs. Follow [**VM/Guest Configuration**](sriov-nics.md#vmguest-configuration) instructions above. In order to use the passthrough device, the VM must run on the tenant node where the device is attached.
