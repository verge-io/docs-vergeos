...
A blank custom VM is created without drives or NICs. (Drives and NICs can be added after VM creation.) A guest operating system can be installed by adding a CD-ROM device with media file set to a bootable install ISO or adding an import disk that has OS installed. See [**Virtual Machine Drives**](vm-drives.md) and [**Virtual Machine NICs**](vm-nics.md) for help in adding drives/NICs.
...
- [**Import**](vm-migration-overview.md)
VM is created from a standard format configuration file (e.g. OVF, VMX, XML, etc) with associated drives created from disk image files, such as \*.VMDK. See: [**VM Migration Methods**](vm-migration-overview.md)
...
**Review [**Virtual Machine Best Practices**](vm-best-practices.md) for important considerations and recommendations regarding creating VMs.**

## Create a new Custom VM

{% hint style="success" %}
Creating a new Custom VM will require an *.iso file or a startup USB image file (e.g. created from a hybrid image) for booting/installing the guest OS. See [**Uploading to the vSAN**](../storage/uploading-files-to-vsan.md) for instructions on uploading these files to the vSAN.
{% endhint %}
...
6. Enter/select fields as appropriate for the desired VM. Virtual Machine Field Descriptions are available [**here**](vm-field-descriptions.md).
...
9. Add drives to the VM as needed. See [**Virtual Machine Drives**](vm-drives.md) for instructions.
10. Add NICs to the VM as needed. See [**Virtual Machine NICs**](vm-nics.md) for instructions.
...
7. Enter/select fields as appropriate for the desired VM. Specific entry fields will vary per recipe; See [**Virtual Machine Field Descriptions**](vm-field-descriptions.md) or consult with the service provider or recipe publisher for more information regarding specific entry fields.
...
10. Typically, VM drive(s) and NIC(s) will be included in the VM recipe. Additional drives and NICs can be added to the VM as needed: See [**Virtual Machine Drives**](vm-drives.md) and [**Virtual Machine NICs**](vm-nics.md) for instructions.
...
13. VM drive(s) and NIC(s) from the original VM will automatically carry over to the cloned VM. Additional drives and NICs can be added to the VM as needed: see [**Virtual Machine Drives**](vm-drives.md) and [**Virtual Machine NICs**](vm-nics.md) for instructions.
