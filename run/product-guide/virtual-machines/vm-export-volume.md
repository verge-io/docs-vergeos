...
- To make the exported data accessible to external systems, such as third-party backup tools or external storage platforms:
    * The volume can be [shared over CIFS or NFS](../nas/nas-shares.md)
    * Data can be [volume synchronized](../nas/volume-syncs.md) to an external system (e.g. NAS appliance) via CIFS/NFS using a mounted [remote volume](../nas/nas-remote-volumes.md)

For detailed configuration instructions, see the KB article: **[How to Configure a Volume for VM Exports](https://app.gitbook.com/s/QZBMFpokMv2vWTIRbFzA/backup-dr/configuring-a-vm-export-volume)**
