...
![IPsec Connect button](../../assets/screenshots/ipsec-connect.png) 
...
Tracing the traffic on the VPN network and associated external and internal networks can often assist troubleshooting efforts by identifying if tunnel traffic is reaching a network, identifying source and destination addresses of packets, etc. **Diagnostics > Diagnostics Query:** ***TCP Dump***.  The [Network Diagnostics Guide](../networks/network-diagnostics.md) provides a reference for using the Network Diagnostics tool.