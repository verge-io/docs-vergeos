...
A secure tunnel connection can be established between VergeOS sites or from a VergeOS site to a third-party WireGuard peer; reference the site-to-site instructions on: [**WireGuard**](wireguard-config.md). Example Site-to-Site WireGuard configurations can be viewed at [**WireGuard Configuration Examples**](wireguard-examples.md).

## VPN for Secure Remote User Access to a VergeOS Site
To configure a secure tunnel for remote users to access a VergeOS system, consult the WireGuard Configuration page: [**WireGuard**](wireguard-config.md) and the [**WireGuard Configuration Examples**](wireguard-examples.md) for sample remote access configurations.
...
Information on creating a VPN tunnel to a third-party IPsec peer can be found at: [**IPsec Configuration**](ipsec.md)