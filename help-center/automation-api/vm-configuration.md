...
**Previous**: VM powered on → [`VM Power Management`](vm-power-management.md)
**Common Next Steps**:
- Advanced operations → [`VM Advanced Operations`](vm-advanced-operations.md)
- Power cycle for changes → [`VM Power Management`](vm-power-management.md)
<!-- END CONTEXT -->
...
### Prerequisites
VM must be created first → See [`VM Creation`](vm-creation-api.md)
...
See [VM Hot-Plug Capabilities](../tenants/vm-hotplug-capabilities.md) for complete details.
{% endhint %}
...
- **VM Creation**: See [`VM Creation`](vm-creation-api.md) for initial VM setup
- **Power Management**: See [`VM Power Management`](vm-power-management.md) for start/stop operations
- **Advanced Operations**: See [`VM Advanced Operations`](vm-advanced-operations.md) for cloning and snapshots
{% endhint %}
