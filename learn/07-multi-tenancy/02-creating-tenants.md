...
| **Confirm Admin User Password** | Re-enter the admin password to confirm it matches                                                                                                                  | Yes      |
...
If a tenant **Proxy** is configured, the UI Management form also offers a **Create a new FQDN** option in place of assigning an external IP. Selecting it opens a **Proxy Tenant Config** page where you choose the proxy's network, the tenant, and the FQDN (the A record pointing to the proxy's IP alias); then click **Skip** at the bottom of the UI Management page to avoid assigning a direct IP. A tenant cannot have both a UI IP address and a proxied FQDN.
...
5. Optionally select **Clone as New Tenant** to strip history, statistics, logs, and snapshots with expiration from the clone
6. Click **Submit**
...
VergeOS Shared Objects let provider and tenant exchange VMs by reference — no full disk copies, and the same mechanism works in both directions. Snapshot the VM, create a Shared Object scoped to the recipient tenant, and the recipient imports it; the underlying vSAN blocks are referenced rather than copied.
{% endhint %}
