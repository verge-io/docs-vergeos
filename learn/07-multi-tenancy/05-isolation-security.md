...
Each tenant receives **dedicated storage volumes** within the vSAN, presented to the tenant as if it had its own storage system. Isolation is enforced logically through container filesystem boundaries — a tenant cannot see or access another tenant's data — while the underlying vSAN deduplicates blocks across the whole cluster for efficiency.
...
| **Parent VergeOS (via OIDC)**  | Tenant authenticates via an OIDC application defined on the parent VergeOS system, so tenant logins are backed by the parent's user directory | Managed service providers who control tenant access centrally |
| **Third-party IdP (via OIDC)** | Tenant integrates with an external identity provider (Okta, Azure AD/Entra, Google Cloud Identity, etc.) via OIDC                             | Enterprise tenants with existing identity infrastructure      |
...
VergeOS provides **Tenant Layer 2 Networks** (recent VergeOS versions) for streamlined VLAN pass-through:
...
{% hint style="warning" %}
**Do Not Tag the Tenant External Network**
