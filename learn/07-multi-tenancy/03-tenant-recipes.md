...
| **Catalog**              | The catalog in which to store the recipe            | Must be a catalog in a local repository — recipes cannot be created directly in remote/Marketplace repositories. See [Recipe Exchange](#recipe-exchange-sharing-between-systems) below |
...
A VergeOS tenant recipe provisions an entire isolated VDC in a single operation — management UI, users, networking stack, firewall rules, storage, and DNS/DHCP — not just VMs or applications. For the scope and capabilities of any VMware or Nutanix automation tool you may be comparing against, consult that vendor's current documentation.
{% endhint %}
