...
| **Spanning tree**    | Disabled on core fabric ports (BPDU guard disable, portfast disable) — STP should not be used for core fabric links. Preferably this traffic wouldn't leave the switch. |
...
A typical VergeOS node has **four network interfaces** — two for core fabric and two for external:

| NICs      | Connection       | Purpose                                                       |
| --------- | ---------------- | ------------------------------------------------------------- |
| NIC 1     | Core Fabric 1    | Primary path for all inter-node traffic                       |
| NIC 2     | Core Fabric 2    | Redundant path for all inter-node traffic                     |
| NIC 3 + 4 | External network | Management UI/API access, user traffic, internet connectivity |
...
graph LR
    subgraph "Node"
        ...
        NIC3["NIC 3<br/>External"]
        NIC4["NIC 4<br/>External"]
    end
...
The DMZ uses the `100.64.0.0/16` address range. Each virtual network's router — external networks, internal networks, the core network, and any nested tenant clouds — has a DMZ interface assigned an address from this subnet:

```mermaid
graph TB
    ISP["ISP / Upstream Router"]
    EXT["External Network Router<br/>Eth0: External IP<br/>Eth1/DMZ: 100.64.0.3"]
    DMZ["DMZ Network<br/>100.64.0.0/16<br/>(Layer 3 routing hub)"]
...
Now that you understand how VergeOS nodes are interconnected, the next topic covers how those nodes are organized into clusters: **[Clusters & Node Types →](05-clusters-nodes.md)**
