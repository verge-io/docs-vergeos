...
- **Tiered storage** -- Drives are organized into a metadata tier (Tier 0, NVMe) and 5 workload tiers (T1–T3 performance, T4 capacity, T5 archive), assigned at install time
...
- **Self-healing** -- Automatic failover on hardware failure with continuous bit-rot detection; rebuild is operator-initiated
...
| **Core Fabric Networks** | Dedicated high-speed inter-node mesh requiring jumbo frames (NIC MTU ~9192, switch ports ≥ 9216) |
...
| **Hypervisor**          | ESXi (proprietary)                                       | AHV (KVM-based)                                          | VergeHV (KVM, integrated into the OS)                                                              | [VM Creation & Lifecycle](../06-virtual-machines/01-vm-creation-lifecycle.md)     |
| **Distributed storage** | vSAN — disk groups, storage policies, witness nodes      | DSF — runs in a CVM on every node, RF2/RF3 per container | VergeFS — integrated into the OS (no CVM), 5 workload tiers + 1 metadata tier, inline global dedup | [vSAN & VergeFS](03-vsan-vergefs.md), [Module 5](../05-storage/README.md) |
| **Networking / SDN**    | NSX (add-on) + vDS for physical NICs                     | Flow (add-on, no built-in DHCP/DNS/routing)              | VergeFabric — built-in firewall, NAT, DHCP/DNS, VPN, micro-segmentation                            | [Core Fabric](04-core-fabric.md), [Module 4](../04-networking/README.md)  |
| **Multi-tenancy**       | vCloud Director / vRA / resource pools                   | Prism Self-Service Projects                              | Native VDCs — full network/storage/management encapsulation                                        | [Module 7](../07-multi-tenancy/README.md)                                                |
| **Per-node overhead**   | vCenter Appliance + vSAN overhead per host + NSX Manager | CVM per node (~20–32 GB RAM, 4–8 vCPU)                   | None beyond 16 GB OS baseline; vSAN runs as an integrated OS service (no CVM)                      | [Hardware Requirements](../02-sizing-design/01-hardware-requirements.md)          |
| **Licensing model**     | Per-socket + per-feature                                 | Per-node + add-on products                               | Single platform license                                                                            | —                                                                             |

## Next Steps

Now that you understand what VergeOS is and its core components, the next topic explores the two deployment models in depth: **[HCI vs UCI →](02-hci-vs-uci.md)**
