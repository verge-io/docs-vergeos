...
| **OS Family**        | Windows, Linux, FreeBSD, or Other                                                                                                                       | Affects QEMU flags and performance optimization                                                                                        |
...
| **Video**            | Display adapter (`std`, `cirrus`, `vmware`, `qxl`, `virtio`)                                                                                            | Standard VESA 2.0 default; choose QXL when using SPICE console; choose VirtIO only when the physical node has a capable GPU (e.g. VDI) |
...
| **HA Group**         | High-availability placement group; group name keeps VMs on separate nodes (negative affinity), and a `+name` prefix co-locates them (positive affinity) | Keeps related VMs on separate nodes                                                                                                    |
...
| **Interface**   | **Virtio** (recommended), Intel E1000, AMD PCnet, Realtek 8139. Imported VMware/Hyper-V VMs may also surface `e1000e`, `vmxnet3`, or `igb` (Intel 82576). |
...
Coming from Nutanix AHV? VergeOS hot-plug works on drives and NICs for VMs; for live CPU/RAM resizing, use **tenant nodes** — the parent system can adjust a tenant node's cores and RAM without restarting the workloads running inside it.
{% endhint %}
