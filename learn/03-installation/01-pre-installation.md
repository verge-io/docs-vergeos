...
- **System reservation per node:** **16 GB OS + 1 GB per 1 TB of raw vSAN storage** (e.g., a node with 4 TB of raw storage reserves at least 20 GB total)
- Plan for workload RAM on top of the system reservation
...
{% hint style="warning" %}
**Critical: Do Not Bond Core Fabric NICs**
...
- **Absolute minimum:** 2 NICs per node -- 1 core fabric + 1 external (a second core fabric NIC is highly recommended, not required)
...
- Prepare a **USB drive for each controller node** (Nodes 1--2 in an N+1 system; Nodes 1--3 in N+2) to store the encryption key (highly recommended). Other (scale-out) nodes do not require a key drive. An operator can instead type the passphrase at the controller-node boot prompt.
...
With all prerequisites verified and documentation in hand, you are ready to proceed to the controller installation: **[Controller Installation →](02-controller-installation.md)**
