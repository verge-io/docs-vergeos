...
For multi-tenant deployments (CSP, MSP, or internal departmental isolation), each tenant runs inside a Virtual Data Center (VDC) backed by one or more **tenant nodes**. Tenant nodes are virtual machines that act as the compute nodes of the tenant's nested VergeOS instance; storage is provisioned from host storage-tier quotas and networking via the tenant's virtual networks.
...
| Core Fabric 1   | VLAN 900              | Untagged access VLAN created on switch; node IPs auto-assigned by VergeOS. Inter-node storage and control traffic |
| Core Fabric 2   | VLAN 901              | Untagged access VLAN created on switch; node IPs auto-assigned by VergeOS. Redundant inter-node traffic           |
...
VergeOS scoping is a single sizing pass: flat 16 GB OS overhead per node plus 1 GB per TB of raw storage. There is no CVM, no separate management appliance, and no per-feature licensing exercise — compute, storage, networking, and multi-tenancy are sized together as one platform.
{% endhint %}
...
- **[Hardware Requirements](01-hardware-requirements.md)** — Verify your node specifications against minimum and recommended requirements.
- **[Reference Architectures](02-reference-architectures.md)** — Review the detailed topology diagrams for your selected architecture.
- **[Module 3: Installation](../03-installation/README.md)** — Proceed to installation preparation and execution.
