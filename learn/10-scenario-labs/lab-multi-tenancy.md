...
# Lab: Multi-Tenancy / MSP Deployment
...
2. **Modify tenant resources:**
   ...
   - Click **Submit**
   - Note: Tenant node cores and RAM are hot-pluggable — the changes take effect immediately, without restarting the tenant or its workloads. The system validates that the requested resources are available on the host/cluster when you submit.
...
4. **Document the networking architecture:**
   ...
   | Tenant External Network    | Connectivity from tenant to host                   | Tenant      |
   | Host External / Virtual IP | Connectivity to outside world                      | Host        |
