...
The Task Engine uses six modular building blocks that can be composed in flexible combinations:
...
| **Scripts**   | Programmable automation building blocks surfaced on the Tasks Dashboard (see Scripts section below) |
...
| Alarms           | Raised               | Send webhook to PagerDuty or on-call rotation |
...
Tasks have an **Enabled** flag; you can disable a task temporarily if you need to verify configuration before activating it, then re-enable it when ready.
...
The Task Engine is built into VergeOS at every level (including inside tenants) — no separate orchestrator appliance, no add-on automation license. Configure event triggers, schedules, and webhooks from the same UI you already use for VMs and storage.
{% endhint %}
