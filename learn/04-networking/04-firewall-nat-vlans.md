...
Advanced rules can filter on connection state (New, Established, Related, Untracked) for stateful packet inspection.
...
For network-wide tracking, enable the network's `statistics` setting to automatically track every rule (refer to the current UI for the exact label).
...
Modern, high-performance VPN protocol with state-of-the-art cryptography and
...
- **Remote user access** — Provide secure VPN access for individual users. VergeOS can auto-generate peer configuration files for download to supported WireGuard client software.
...
With firewall rules, NAT, VLANs, and VPN configured, explore the hands-on lab to put these concepts into practice: **[Lab: Network Configuration →](lab.md)**
