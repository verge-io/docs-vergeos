...
The core network is never exposed to external traffic. It is the most performance-critical network in a VergeOS system because vSAN throughput depends directly on inter-node communication speed. For a deep dive into the core fabric, see [Module 1: Core Fabric & Networking](../01-architecture/04-core-fabric.md).
...
2. A **core network** is created for the tenant's vSAN and inter-node traffic
...
Coming from VMware? VergeFabric is one integrated SDN stack — physical networks for switch/NIC connectivity, core for inter-node traffic, external networks for upstream, DMZ for L3 routing/firewall, and internal networks with built-in DHCP/DNS/firewall — instead of a separately licensed and managed SDN layer.
{% endhint %}

{% hint style="info" %}
**Nutanix Bridge**

Coming from Nutanix? VergeFabric bundles micro-segmentation, distributed firewall, DHCP, DNS, and routing into every internal network — no separate license or external services required.
{% endhint %}
...
Now that you understand the VergeOS network types and how traffic flows through the system, the next topic covers how to connect VergeOS to your upstream infrastructure: **[External Networks →](02-external-networks.md)**
