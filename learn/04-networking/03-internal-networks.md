...
```mermaid
graph LR
    INT1["Internal Net A<br/>10.10.1.0/24<br/><br/>Route rule:<br/>→ Net B via DMZ"] -->|"transit"| DMZ["DMZ<br/>(transit fabric)"]
    DMZ -->|"transit"| INT2["Internal Net B<br/>10.10.2.0/24<br/><br/>Route rule:<br/>→ Net A via DMZ"]
...
- **Direction:** Outgoing
- **Protocol:** ANY
- **Source:** My Network Address
- **Destination:** Other Network Address — select the destination internal network
- **Target:** Other Network DMZ IP — select the destination internal network
...
In VergeOS, an internal network is a single object that bundles segment + router + DHCP + DNS + default-deny firewall. The DMZ network is the single L3 routing point for inter-network traffic.
{% endhint %}
...
With internal networks configured, the next topic covers how to secure and control traffic flow with firewall rules, NAT, and VLANs: **[Firewall Rules, NAT & VLANs →](04-firewall-nat-vlans.md)**
