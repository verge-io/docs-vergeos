...
4. The NIC appears in the guest immediately when hot-plug is enabled (the default) — a guest-OS rescan may be needed on some operating systems. Power-cycle only if hot-plug is disabled.
...
Unlike platforms that rely on memory ballooning to reclaim unused guest memory, **VergeOS intentionally does not use ballooning**. This design choice provides:
...
**Advanced Configuration Options for macOS clients:** For better macOS compatibility, add the macOS-oriented directives (including `vfs objects = fruit streams_xattr` and the related `fruit:*` options) under **Advanced Configuration Options** in the NAS CIFS settings (**NAS → CIFS**). This enables native Apple SMB extensions.
...
The following break-points are **training guidance** for planning, not documented thresholds. The documented values are the **80% subscription high-usage default** (used by the seeded vSAN/storage-tier high-usage alerts) and the **90% `sync_max_usage`** threshold at which vSAN throttles writes and marks the tier `outofspace`.
