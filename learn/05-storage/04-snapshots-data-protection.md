...
VergeOS snapshots leverage the **VergeFS hash-based architecture** to deliver nearly instant, space-efficient, point-in-time captures. Because VergeFS already stores data as deduplicated 64KB blocks identified by content-addressable hashes, a snapshot is simply a **point-in-time capture of hash-tree references** -- it preserves the root hashes that identified each file and VM disk at that instant.
...
- **Instant creation** -- capturing hash-tree root references is nearly instantaneous regardless of data size
...
You already know application-consistent snapshots require a guest agent. In VergeOS the same agent covers Linux (fsfreeze) and Windows (VSS), and the same Quiesce option appears for manual VM snapshots, scheduled VM snapshots via profile, partial system snapshots (quiesce tags), and NAS volume snapshots.
{% endhint %}
...
| Profile                  | Schedule                                                                                                                |
| ------------------------ | ----------------------------------------------------------------------------------------------------------------------- |
| **System Snapshots**     | Hourly (retained 3 hours, preconfigured Immutable), daily at midnight (retained 3 days), daily at noon (retained 1 day) |
