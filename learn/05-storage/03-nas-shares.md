...
Coming from VMware? On VergeOS, file storage isn't a separate appliance or add-on service — the NAS is a built-in VM recipe sitting on the same vSAN as your VMs, inheriting dedup, snapshots, and HA from the platform.
{% endhint %}

{% hint style="info" %}
**Nutanix Bridge**

Coming from Nutanix? On VergeOS, the NAS is a single VM recipe per system or tenant — HA comes from the underlying cluster, and the volume sits on an operator-selected preferred vSAN tier (no policy/access-pattern-based tier movement; changing the preferred tier triggers a live background migration).
{% endhint %}
