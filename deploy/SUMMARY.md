# Table of contents

* [Plan and deploy](README.md)

## Implementation guide

* [Introduction](implementation-guide/intro.md)
* [Core concepts](implementation-guide/concepts.md)
* [Node sizing](implementation-guide/sizing.md)
* [Network design](implementation-guide/network-design.md)
* [Pre-installation checklist](implementation-guide/pre-installation.md)
* [Creating bootable installation media](implementation-guide/install-media.md)
* [Installation guide](implementation-guide/installation-guide.md)
* [Core fabric MTU when switch MTU is 9000](implementation-guide/core-fabric-mtu-switch-9000.md)
* [Post-installation configuration](implementation-guide/post-installation.md)
* [Scale-out node installation guide](implementation-guide/scale-out-nodes.md)
* [Compute only node installation guide](implementation-guide/compute-nodes.md)
* [Storage node installation guide](implementation-guide/storage-nodes.md)
* [Switch configuration guide](implementation-guide/switch-configuration.md)

## Reference architectures

* [Edge cluster deployments](reference-architecture/edge.md)
* [Multi-tenant deployments](reference-architecture/csp.md)
* [High-performance cluster deployments](reference-architecture/data-science.md)
* [VergeOS homelab reference architecture](reference-architecture/homelab.md)
