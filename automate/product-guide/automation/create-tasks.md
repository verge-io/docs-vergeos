...
{% hint style="success" %}
**New tasks are *Enabled* by default. You can disable a task if you need time to verify its configuration or delay deployment.**
...
{% hint style="success" %}
**The instructions below begin at the task dashboard, which appears immediately after task creation. To return to the dashboard later: go to **System** > **Tasks Dashboard** > **Tasks**, then **double-click** the task in the list.**
...
{% hint style="success" %}
**The card displays the number of event triggers currently associated with the task.**
...
3. Select a **Schedule** from the dropdown list.  
   VergeOS includes several pre-installed schedules. To create a custom schedule, see the **[Schedules Guide](schedules.md)**.
...
- [Using Webhooks](webhooks.md) 
- [Defining Schedules](schedules.md) 
