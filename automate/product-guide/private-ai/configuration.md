...
{% hint style="info" %}
**GPU devices are distributed from pools called [Resource Groups](https://app.gitbook.com/s/pODKGSQETqL1gSqyxIq3/product-guide/system/device-pass-overview#resource-groups).**
...
{% hint style="success" %}
**The model may take several minutes or more to download.  The model dashboard (Click *Models* on the left menu and double-click the model) will indicate a "Downloading" status and progress while the model is actively transferring.  *Status* will change to "Online" when the model is fully downloaded, initialized and ready for use.**
...
{% hint style="info" %}
**Availability of the *Think* feature depends on model support.**
