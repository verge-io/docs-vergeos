...
description: >-
  Guide to routing Layer 3 IP traffic for Layer 2 physical networks in VergeOS,
  enabling communication between physical devices and internal VM workloads on
  different subnets.
tags:
  - network
  - networking
...
2. [Create IP block on the External network](network-blocks.md#creating-a-network-block) that will route the traffic (Likely default External)
...
Introduction to Network Blocks : [Network Blocks Overview](network-blocks.md)\
Routing Basics : [Routing Internal Networks](routing-between-internal-vergeio-networks.md)\
Network Rules : [VergeOS Product Guide - Network Rules](https://app.gitbook.com/s/pODKGSQETqL1gSqyxIq3/networking/network-rules)\
Network Troubleshooting : [VergeOS Product Guide - Network Troubleshooting](https://app.gitbook.com/s/pODKGSQETqL1gSqyxIq3/networking/net-troubleshooting)
{% endhint %}
...
* If Native layer 2 access is required inside tenant networks, see [Provide Layer 2 Access to a Tenant](../tenants/provide-layer2-to-tenant.md) to create a Virtual Switch Port connecting the tenant network to the Layer 2 External interface outside of the tenant.
