...
description: >-
  Guide for determining the optimal number of tenant nodes, compute resource
  allocation, and physical placement strategies for VergeOS tenant deployments.
tags:
  - workloads
  - tenants
  - design
...
Before using this guide, you should have a general foundational understanding of tenant concepts; refer to the [Tenants Overview](https://app.gitbook.com/s/pODKGSQETqL1gSqyxIq3/tenants/overview) if you are new to VergeOS tenants.
...
For detailed procedures on increasing tenant resources, refer to the [Increase Tenant Resources](https://app.gitbook.com/s/pODKGSQETqL1gSqyxIq3/tenants/add-tenant-resources) documentation.
...
Multiple tenant nodes are needed when a tenant requires more compute resources than the cluster will allow within a single machine. The amount of memory and number of cores that can be allocated to a single tenant node is limited by cluster settings: [***Max RAM per machine***_ and _***Max cores per machine***](https://app.gitbook.com/s/pODKGSQETqL1gSqyxIq3/system-administration/cluster-settings).
...
* Host cluster settings allow for _Max RAM per machine_: 64GB RAM and _Max cores per machine_: 16
...
* Host cluster settings allow for _Max RAM per machine_: 128GB RAM and _Max cores per machine_: 16
...
{% hint style="success" %}
[**This KB article**](../automation-api/determine-node-where-vm-runs.md) **provides information about using HA Groups for node anti-affinity**
{% endhint %}
...
* Applicable Host clusters:
  * Standard (mid-range processors, standard processor/core ratio); _Max RAM per machine_: 64GB RAM and _Max cores per machine_: 16
  * vGPU Cluster (high-end processors, vGPU devices); _Max RAM per machine_: 64GB RAM and _Max cores per machine_: 16
  * High-Performance Cluster (high-end processors, memory dense) _Max RAM per machine_: 128 RAM and _Max cores per machine_: 16
...
**Hardware Matching:** Each tenant node is placed on physical infrastructure that matches its workload requirements (tenant node _Cluster_ setting), optimizing both performance and cost.
...
* Host cluster settings allow for _Max RAM per machine_: 96GB RAM and _Max cores per machine_: 16
