...
Before using this guide, you should have a general foundational understanding of tenant concepts; refer to the [Tenants Overview](https://app.gitbook.com/s/pODKGSQETqL1gSqyxIq3/product-guide/tenants/overview) if you are new to VergeOS tenants.
...
For detailed procedures on increasing tenant resources, refer to the [Increase Tenant Resources](https://app.gitbook.com/s/pODKGSQETqL1gSqyxIq3/product-guide/tenants/add-tenant-resources) documentation.
...
Multiple tenant nodes are needed when a tenant requires more compute resources than the cluster will allow within a single machine.  The amount of memory and number of cores that can be allocated to a single tenant node is limited by cluster settings: [***Max RAM per machine*** and *Max cores per machine***](https://app.gitbook.com/s/pODKGSQETqL1gSqyxIq3/product-guide/system/cluster-settings). 
