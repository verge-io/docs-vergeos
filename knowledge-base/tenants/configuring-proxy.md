...
description: >-
  How to configure a proxy connection to serve multiple tenant environments
  using a single IP address mapped to FQDN hostnames.
tags:
  - proxy setup
  - network
  - alias
...
Submit the settings but _**DO NOT RESTART THE NETWORK OR APPLY RULES YET!**_
{% endhint %}
...
2.  From the same external network:
    ...
    * Create a new rule that looks like the following image:
    ...
    ![proxy\_accept\_rule.png](../.gitbook/assets/proxy_accept_rule.png)
...
7.  Select the tenant network (highlighted) from the tenant dashboard.
    ...
    ![tenant\_apply\_rules.png](../.gitbook/assets/tenant_apply_rules.png)
