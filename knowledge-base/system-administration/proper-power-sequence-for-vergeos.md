...
5. The system will automatically perform integrity checks of each storage tier.
    ...
    {% hint style="success" %}
    **Storage Tier Integrity Checks**

    See [vSAN Tier Status (Journal Walks)](../storage-vsan/understanding-journal-walks-and-vsan-tier-status.md) for information related to these automatic integrity checks (journal walks).
    {% endhint %}
...
Extra care should be taken when powering on a cluster after an uncontrolled shutdown (e.g. an abrupt power loss, UPS exhaustion, or facility failure). See [Cluster Recovery After Full Power Outage](cluster-recovery-after-power-outage.md) for the full recovery procedure, including pre-power-on checks, vSAN integrity verification, and prevention guidance.
{% endhint %}
