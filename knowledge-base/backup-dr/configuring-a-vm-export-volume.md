...
description: >-
  How to configure the VergeOS NAS VM Export Volume for backup purposes,
  including CIFS/NFS sharing, scheduled exports, and synchronization to external
  storage.
tags:
  - vm export
  - export
...
The VergeOS NAS supports a volume type specifically for exporting selected virtual machines (VMs). The _VM Export volume_ provides a controlled way to export selected VMs to a NAS volume that can then be synchronized to a remote storage system (e.g. existing NAS appliance) or shared via CIFS or NFS for access by external backup tools or other applications.
...
2. Configure the volume:
   ...
   * **Filesystem Type**: _**Verge.io VM Export**_
   * **VM configuration output file format**: Select the format used for the exported VM configuration file.
     * _**Verge.io Virtual Machine (.ybvm)**_ — A human‑readable, JSON‑based format that is non‑binary and non‑proprietary. This format is ideal for portability, transparency, and interoperability with external tools or workflows that prefer open, inspectable configuration data.
     * _**Open Virtualization Format (.ovf)**_ — A widely supported industry-standard format used by many third‑party virtualization platforms and import utilities.
   * **Quiesced**: Typically should be selected to provide application-consistent VM snapshots. ⚠️ [**VM Guest Agent**](https://app.gitbook.com/s/pODKGSQETqL1gSqyxIq3/virtual-machines/vm-guest-agent) **must be installed and registered to provide a quiesced VM snapshot.**
...
{% hint style="success" %}
**You can quickly view the contents of the export volume using the&#x20;**_**Browse**_**&#x20;option on the left menu.**
{% endhint %}
...
Instructions for creating an NFS share can be found [here](https://app.gitbook.com/s/pODKGSQETqL1gSqyxIq3/nas/nas-shares#create-an-nfs-share).
...
1. Create a [Remote Volume](https://app.gitbook.com/s/pODKGSQETqL1gSqyxIq3/nas/nas-remote-volumes) to mount the external volume to the VergeOS NAS (requires standard NFS or CIFS access)
2. Create a [Volume Sync](https://app.gitbook.com/s/pODKGSQETqL1gSqyxIq3/nas/volume-syncs) to synchronize data to the Remote Volume. Volume syncs can be started on-demand manually and can also be scheduled using the Start Profile setting.
...
VergeOS includes multiple pre-installed schedules (e.g., "Daily at midnight"). Refer to the [Schedules Guide](https://app.gitbook.com/s/sppYQkyIET58BuAo0kqm/automation/schedules) for instructions on creating custom schedules.
{% endhint %}
...
4. Configure the new task fields:
   * **Name**: provide a descriptive name, e.g. _start-vm-export_
   * **Object Type**: _**VM Export**_ (pre-selected when accessed from the volume)
   * **Object**: select the export volume (pre-selected when accessed from the volume)
   * **Action**: _**Start Export**_
